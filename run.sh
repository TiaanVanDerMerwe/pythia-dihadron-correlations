...
module load software/pythia-8317
export PYTHIA8=/opt/exp_soft/pythia-8317
...
mkdir -p logs
mkdir -p pythiaData/${COM}/alice

echo "[$BIN:$SEED] Started: $(date)"
echo "[$BIN:$SEED] Host:    $(hostname)"

./bin/chargedGenerationCorrelation ${COM} ${NEVENTS} ${POW} ${SEED} ${BIN}
