...
def save_integrated_yields_csv(slice_results, out_dir, trig_pt_range):
    ...
    header = [
        "# Per-trigger integrated near/away yields vs pT_assoc",
        f"# trigger pT : {tlo:.1f} – {thi:.1f} GeV/c",
        f"# jackknife blocks : {N_JACKKNIFE_BLOCKS}",
        "# Near side : -pi/2 < Delta-phi <= pi/2",
        "# Away side :  pi/2 < Delta-phi <= 3pi/2",
...
def plot_dphi_slices(
    ...
    fig.suptitle(
        r"FSR & No ISR Dihadron $\Delta\phi$ Distributions" + "\n"
...
def plot_integrated_yields(
    ...
    if plot_ratio:
        ...
        fig, axes_1d = plt.subplots(1, 2, figsize=(10, 4), gridspec_kw={"wspace": 0.3})
