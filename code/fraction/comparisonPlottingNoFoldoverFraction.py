...
STYLE = {
    'nofsr':         dict(label = "No FSR", color='#E8700A', marker='^', ls='-',  lw=1.4),
    'fsr':           dict(label = "FSR", color='#4453FF', marker='^', ls='-',  lw=1.4),
}

EXP_STYLE = dict(color='black', marker='*', ms=8, ls='-',
                 markerfacecolor='black', markeredgewidth=0.8,
                 zorder=10, label='Test data')
...
def load_dphi_csvs(base_dir, subdirs):
    ...
    for sd in subdirs:
        ...
        for fpath in files:
            ...
            trig_lo, trig_hi, zT_lo, zT_hi = pt
...
def plot_dphi_comparison(all_data, pt_key, exp_dir, save_dir,
    ...
    exp_phi, exp_y, exp_y_err_pos, exp_y_err_neg = load_exp_data(exp_dir, trig_lo, trig_hi, zT_lo, zT_hi)
    ...
    for sd in subdirs:
        ...
        bin_width = np.diff(phi, append=phi[-1] + (phi[-1] - phi[-2]))
    ...
    if exp_phi is not None:
        bin_width = np.diff(exp_phi, append=exp_phi[-1] + (exp_phi[-1] - exp_phi[-2]))
        ...
        ax_top.errorbar(
            ...
            fmt="none", ms=MS, color=EXP_STYLE['color'], linestyle=st['ls'],
