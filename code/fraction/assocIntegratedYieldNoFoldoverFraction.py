...
def integrate_yield(phi_centers, phi_proj, jackknife_projs, zt_range,
                    near_lo=NEAR_SIDE_LO, near_hi=NEAR_SIDE_HI,
                    away_lo=AWAY_SIDE_LO, away_hi=AWAY_SIDE_HI):
...
def load_star_data(trig_pt_range, data_dir='.'):
    ...
    for side in ('near', 'away'):
        fname = os.path.join(data_dir,
                             f"STAR_{side}_{tlo:.0f}-{thi:.0f}.csv")
...
def plot_dphi_slices(slice_results, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
    ...
    fig.suptitle(
        rf'Dihadron $\Delta\phi$ Distributions' + '\n'
...
_STAR_STYLE = {
    'dAu':        dict(fmt='s', color='#E8700A', label=r'd+Au',           ms=7),
    'AuAu_0_5':   dict(fmt='D', color='#CC2222', label=r'Au+Au 0–5%',     ms=7),
    'AuAu_20_40': dict(fmt='^', color='#229922', label=r'Au+Au 20–40%',   ms=7),
}


def plot_integrated_yields(slice_results, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
                           data_dir='.', save_path=None):
    """
    ...
    panels = [
        (axes[0, 0], axes[1, 0],
         y_near, err_near, 'near',
         'Near side',
         rf'$-0.63 < \Delta\phi < 0.63$ rad',
         r'$D^{\rm near}(z_T)$'),
        (axes[0, 1], axes[1, 1],
         y_away, err_away, 'away',
         'Away side',
         rf'$2.51 < \Delta\phi < 3.77$ rad',
         r'$D^{\rm away}(z_T)$'),
    ]
    ...
    for ax_top, ax_rat, yields, errs, side_key, label, phi_label, ylabel in panels:
        ...
        # ── Pythia points ─────────────────────────────────────────────────
        ax_top.errorbar(zt_centers, yields, xerr=xerr, yerr=errs,
                        fmt='o', color=pythia_color, ms=7, markeredgewidth=0.6,
                        ecolor=pythia_color, elinewidth=1.5, capsize=4,
        ...
        if A_py is not None:
            ...
            ax_top.plot(x_fit, y_fit, '--', color=pythia_color, lw=1.6, alpha=0.85,
                        label=rf'Pythia fit  $b={b_py:.2f}\pm{sb_py:.2f}$',
                        zorder=5)
            ax_top.fill_between(x_fit, y_fit * (1 - rel_band),
                                y_fit * (1 + rel_band),
                                color=pythia_color, alpha=0.15, zorder=4)
        ...
        for sys_key, sty in _STAR_STYLE.items():
            ...
            if sys_data is None:
                continue
            ax_top.errorbar(sys_data['zt'], sys_data['y'],
                            xerr=0.05,              # half of 0.1 zT bin width
                            yerr=sys_data['err'],
                            fmt=sty['fmt'], color=sty['color'],
                            ms=sty['ms'], markeredgewidth=0.6,
                            ecolor=sty['color'], elinewidth=1.5, capsize=4,
                            label=sty['label'], zorder=6)
        ...
        if dau_data is not None:
            ...
            for zt_d, y_d, e_d in zip(dau_data['zt'],
                                       dau_data['y'],
                                       dau_data['err']):
            ...
            if rat_zt:
                ax_rat.errorbar(rat_zt, rat_y,
                                xerr=rat_xerr, yerr=rat_err,
                                fmt='s', color=_STAR_STYLE['dAu']['color'],
                                ms=6, markeredgewidth=0.6,
                                ecolor=_STAR_STYLE['dAu']['color'],
                                elinewidth=1.5, capsize=4,
                                label='d+Au / Pythia', zorder=4)
            ax_rat.legend(fontsize=8, framealpha=0.2, loc='upper right')
        else:
            ax_rat.text(0.5, 0.5, 'd+Au data not loaded',
                        ha='center', va='center',
    ...
    for res in (r for r in slice_results if r is not None):
        ...
        jk_tot   = np.array([
            float((np.where(res['jackknife_projs'][k] > 0,
                            res['jackknife_projs'][k], 0.0) * bw).sum() / delta_zt)
            for k in range(n_blocks)
        ])
        factor = (n_blocks - 1) / n_blocks
...
def main():
    POW  = 3
    BASE = f'pythiaData/200/star/eta1'
