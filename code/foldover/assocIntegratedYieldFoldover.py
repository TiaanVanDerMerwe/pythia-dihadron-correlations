...
def plot_dphi_slices(slice_results, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
    ...
    for i, res in enumerate(valid):
        ...
        ax.step(phi, sig, where='mid', color=color,
                linestyle="-", linewidth=2, label=f'Pythia  ZYAM={res["background"]:.4f}', zorder=4)
        ax.errorbar(phi, sig, yerr = err, fmt = 'none', ls='none', color=color, ms=5, lw=1.4, zorder=4)
...
def plot_integrated_yields(slice_results, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
    ...
    panels = [
        (axes[0, 0], axes[1, 0], y_near, err_near,
         False, 'Near side', r'$|\Delta\phi| < 1$ rad'),
        (axes[0, 1], axes[1, 1], y_away, err_away,
         True,  'Away side', r'$1 < |\Delta\phi| < \pi$ rad'),
    ]
...
def main():
    ...
    print("Saving integrated yields to CSV ...")          # ← add
    save_yield_csv(slice_results, trig_range, out_dir)   # ← add
