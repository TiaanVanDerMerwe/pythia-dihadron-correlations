...
std::string makeFilename(double COM, double POW, int SEED, double pTHatMin,
                         double pTHatMax) {
  std::ostringstream oss;
  oss << "pythiaData/" << std::to_string((int)COM) << "/alice/dihadron_pow"
      << std::to_string((int)POW) << "_pT" << std::to_string((int)pTHatMin)
      << "to" << std::to_string((int)pTHatMax) << "_seed"
      << std::to_string((int)SEED) << ".csv";
  return oss.str();
}
...
//    -> pythiaData/200/(cms or star or
//    alice)/dihadron_pow2_pT10to20_seed1_particle_counts.txt
...
int main(int argc, char *argv[]) {
  ...
  if (iBin < 0 || iBin >= nBin) {
    std::cerr << "bin_index " << iBin << " out of range [0, " << nBin - 1
              << "]\n";
    return 1;
  }
  ...
  const std::vector<std::pair<double, double>> trigRanges = {{19.2, 24.0}};
  ...
  // ── Output file ──────────────────────────────────────────────────
  std::string fname =
      makeFilename(COM, POW, SEED, pTMin, pTMax < 0 ? 999 : pTMax);
  ...
  std::vector<double> rangeWeightSums(
      trigRanges.size(), 0.0); // Sums of trigger weights that fall in a
                               // pTtrig range (sums to triggerWeightSum)
  ...
  for (int iEvent = 0; iEvent < NEVENTS; ++iEvent) {
    ...
    for (int i = 0; i < pythia.event.size(); ++i) {
      ...
      if (pt >= TRIG_PT_MIN && pt <= TRIG_PT_MAX && eta >= TRIG_ETA_MIN &&
          eta <= TRIG_ETA_MAX) {
    ...
    for (int iTrig : triggerIndices) {
      ...
      for (int i = 0; i < pythia.event.size(); ++i) {
        ...
        if (!assoc.isFinal() || !assoc.isCharged() || !assoc.isHadron())
          continue;

        double pt = assoc.pT(), eta = assoc.eta();
        if (pt >= ASSOC_PT_MIN && pt < ASSOC_PT_MAX && eta >= ASSOC_ETA_MIN &&
            eta <= ASSOC_ETA_MAX) {

          out << globalEvent << "," << eventWeight << "," << iTrig << ","
              << trigger.pT() << "," << trigger.eta() << "," << trigger.phi()
              << "," << i << "," << assoc.pT() << "," << assoc.eta() << ","
              << assoc.phi() << "\n";
          ++pairCount;

          // Count associate by particle name (once per trigger–assoc
          // pair)
  ...
  for (int r = 0; r < (int)trigRanges.size(); ++r) {
    out << "# triggerWeightSum_" << trigRanges[r].first << "to"
        << trigRanges[r].second << ": " << rangeWeightSums[r] << "\n";
  }
  out.close();
  ...
  if (!cntOut.is_open()) {
    ...
    cntOut << "#   pT in [" << ASSOC_PT_MIN
           << ", pTtrig] GeV  (dynamic upper bound)\n";
    ...
    cntOut << std::left << std::setw(30) << "particle_name" << std::right
           << std::setw(12) << "count" << "\n";
    cntOut << std::string(44, '-') << "\n";
    for (auto &[name, cnt] : sortedEntries(triggerParticleCounts))
      cntOut << std::left << std::setw(30) << name << std::right
             << std::setw(12) << cnt << "\n";
    cntOut << "\n";

    // ── Associate table ───────────────────────────────────────────
    long long totalAssoc = 0;
    for (auto &[n, c] : assocParticleCounts)
      totalAssoc += c;
